...
app.post("/solve", async (req, res) => {
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        if (process.env.GROQ_API_KEY) { const gr = await groq.chat.completions.create({ model: "openai/gpt-oss-20b", messages: [{ role: "system", content: "You are Dragon AI, created by Zakaria. Be smart, accurate, friendly, and helpful. For math, solve step-by-step and identify the rule or method. Match Bangla, English, or Banglish." }, ...messages.map(m => ({ role: m.role === "model" ? "assistant" : "user", content: m.content }))] }); return res.json({ answer: gr.choices[0].message.content }); }
      try {
